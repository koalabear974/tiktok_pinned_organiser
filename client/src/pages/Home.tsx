...
/**
 * Home page: the main page of the app that composes all pieces together.
 */
export default function Home() {
  ...
  const filters = useMemo(() => {
    ...
    if (searchQuery) {
      chips.push({ label: `Search: "${searchQuery}"`, onDismiss: () => handleSearchChange('') });
    }
    if (selectedCategory === 'uncategorized') {
      chips.push({ label: 'Uncategorized', onDismiss: () => handleSelectCategory(null) });
    } else if (typeof selectedCategory === 'number') {
      const cat = categories.find((c) => c.id === selectedCategory);
      if (cat) chips.push({ label: `Category: ${cat.name}`, onDismiss: () => handleSelectCategory(null) });
    }
    return chips;
  ...
  return (
    <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
      ...
      <DragOverlay dropAnimation={null} modifiers={[snapCenterToCursor]}>
        {isDragActive && <DragPreview count={dragVideoIds.length} thumbnailUrl={dragThumbnailUrl} />}
      </DragOverlay>
